...
#ifndef AOWLSPT_KEYCODE_H
#define AOWLSPT_KEYCODE_H

#include <stdint.h>
#include <string.h>

#define AOWL_KC_UNKNOWN (-1)      /* not a key: a refusal                     */
#define AOWL_KC_UNBOUND (-1)      /* the same sentinel, used as "no hotkey"   */
#define AOWL_KC_COUNT   328

typedef struct AowlKeyCodeEntry { const char* name; int32_t ordinal; } AowlKeyCodeEntry;

static const AowlKeyCodeEntry AOWL_KEYCODES[AOWL_KC_COUNT] = {
    { "None", 0 },
    { "Backspace", 8 },
    { "Delete", 127 },
    { "Tab", 9 },
...
static int aowl_kc_fold(int c) {
    return (c >= 'A' && c <= 'Z') ? (c - 'A' + 'a') : c;
}

/* Case-insensitive, because a key name is typed by a human into config.json. */
static int32_t aowl_keycode_of(const char* name) {
    int i;
    if (!name || !name[0]) return AOWL_KC_UNKNOWN;
    for (i = 0; i < AOWL_KC_COUNT; i++) {
        const char* a = AOWL_KEYCODES[i].name;
        const char* b = name;
        while (*a && *b && aowl_kc_fold((unsigned char)*a) == aowl_kc_fold((unsigned char)*b)) { a++; b++; }
        if (!*a && !*b) return AOWL_KEYCODES[i].ordinal;
    }
    return AOWL_KC_UNKNOWN;
}

/* The canonical name for an ordinal, or NULL. NULL is again a refusal: an
 * ordinal we cannot name is not printed as a number pretending to be a key. */
static const char* aowl_keycode_name(int32_t ordinal) {
    int i;
    if (ordinal == AOWL_KC_UNBOUND) return 0;
    for (i = 0; i < AOWL_KC_COUNT; i++)
        if (AOWL_KEYCODES[i].ordinal == ordinal) return AOWL_KEYCODES[i].name;
    return 0;
}

#endif /* AOWLSPT_KEYCODE_H */
